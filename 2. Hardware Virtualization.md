...
We can also run commands from outside the container. If we don't command it will run directly.
...
If command is run from the container, it only sees its own processes.
ps fauxwww -

For example, an index is created for each container at lxc and below, as follows. In this way, we can set each container as 1 GB of mem or restrict all containers to 10 GB of ram in total.
...
- >> Looks at the download file above. The download file has the required configuration for the container.

When LXC downloads, it downloads the following files to cache. When we say LXC destroy, cache does not get infected, only the file under var / lib / lxc / container_name is deleted.
...
-rw-r - r-- 1 root root 72 Jul 20 01:25 config.2nd
...
We have our lxc container on another machine in another linux machine
We can do this by moving the file / var / lib / lxc / container_name.

/ usr / share / lxc / templates - >> There are scripts such as centos, ubuntu in the folder and we can set up ubuntu container by passing these parameters. However, if we want to establish a centos for example, we need to install yum on debian. The important thing here is that the lxc version is the same for another machine we will install.
